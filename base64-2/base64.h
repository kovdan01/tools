...
std::vector<Byte> decode(const std::string& b64_string)
{
    if (b64_string.size() % 4 != 0)
        throw std::invalid_argument("Invalid base64 (length must be multiple of 4)");
    static const std::map<char, std::uint8_t> char_to_num =
    {
        { 'A', 0 },
        { 'B', 1 },
        { 'C', 2 },
    ...
    if (b64_string.empty())
        return {};
    std::size_t group_num = b64_string.size() / 4;
    std::vector<Byte> result;
    if (b64_string[b64_string.size() - 1] == '=')
    {
        if (b64_string[b64_string.size() - 2] == '=')
           result.resize(3 * (group_num - 1) + 1);
        else
           result.resize(3 * (group_num - 1) + 2);
    }
    else
    {
        result.resize(3 * group_num);
    }
    std::size_t i = 0;
    ...
    try
    {
        for (; i < group_num - 1; ++i)
        {
            c1 = char_to_num.at(b64_string[i * 4]);
            c2 = char_to_num.at(b64_string[i * 4 + 1]);
            c3 = char_to_num.at(b64_string[i * 4 + 2]);
            c4 = char_to_num.at(b64_string[i * 4 + 3]);
            result[3 * i] = (c1 << 2) | (c2 >> 4);
            result[3 * i + 1] = ((c2 & 0b001111) << 4) | (c3 >> 2);
            result[3 * i + 2] = ((c3 & 0b000011) << 6) | c4;
        }
