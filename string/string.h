...
namespace my
{
    class string
    {
        ...
        static constexpr std::ratio<3, 2> mul_coeff{};
    ...
    public:
        ...
        char& operator[](std::size_t pos) { return *(_data + pos); }
        const char& operator[](std::size_t pos) const { return *(_data + pos); }
        ...
        char* c_str() const noexcept { return _data; }
        ...
        private:
            const char* _sym;
        public:
            const_iterator(const char* sym) noexcept : _sym(sym) {}
            ...
            const_iterator& operator++() { ++_sym; return *this; }
            const_iterator operator++(int) { auto old = *this; ++(*this); return old; } //NOLINT (cert-dcl21-cpp)
            const_iterator& operator--() { --_sym; return *this; }
            const_iterator operator--(int) { auto old = *this; --(*this); return old; } //NOLINT (cert-dcl21-cpp)
            const_iterator& operator+=(difference_type n) { _sym += n; return *this; }
            const_iterator& operator-=(difference_type n) { _sym -= n; return *this; }
        ...
        std::size_t length() const noexcept { return _size; }
